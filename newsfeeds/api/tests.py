...
class NewsFeedApiTests(TestCase):
    ...
    def setUp(self):
        self.clear_cache()
        self.linghu = self.create_user('linghu', "linghu@hi.com")
        self.linghu_client = APIClient()
        self.linghu_client.force_authenticate(self.linghu)

        self.dongxie = self.create_user('dongxie', "donxie@hi.com")
    ...
    def test_tweet_cache(self):
        ...
        self.create_newsfeed(user=self.dongxie, tweet=tweet)
    ...
    # to check if newsfeed list is complete
    ...
    def test_redis_list_limit(self):
        ...
        # reverse the list, latest the newest
        newsfeeds = newsfeeds[::-1]

        # check if cached list len is equal to cache limit
        cached_newsfeeds = NewsFeedService.get_cached_newsfeeds(self.linghu.id)
        self.assertEqual(len(cached_newsfeeds), list_limit)

        # check if db has the entire list
        queryset = NewsFeed.objects.filter(user_id=self.linghu.id)
