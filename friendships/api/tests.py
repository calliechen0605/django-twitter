...
class FriendshipApiTests(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(2):
            follower = self.create_user('dongxie_follower{}'.format(i), 'linghu@gmail.com')
            Friendship.objects.create(from_user=follower, to_user=self.dongxie)

        for i in range(3):
            following = self.create_user('dongxie_following{}'.format(i), 'linghu@gmail.com')
    ...
    def test_followers_pagination(self):
        ...
        #anoymous user has no "has_followed"
        response = self.anonymous_client.get(url,{'page' : 1})
